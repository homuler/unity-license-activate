...
async function start(email, password, alf, verification, emailPassword) {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelectorAll('input[class="verify_code"], input[name="licenseFile"]').length);

    const needVerify = await page.$('input[class="verify_code"]');
